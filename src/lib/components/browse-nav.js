...
export class BrowseNav {
    ...
    async keyboardHandler(e){
        ...
        if (e.keyCode === 38) {
            ...
            isNextExist = util.goToHorizontalSibling(focusItem.parentElement.previousElementSibling, this.setFocus.bind(this));
            if(isNextExist&&!util.isVisible(focusItem.parentElement.previousElementSibling, focusItem.parentElement.parentElement)){
                util.slidePrevOrNext(util.getDistance(focusItem.parentElement, true), focusItem.parentElement.parentElement, true);
            }
        }
        else if (e.keyCode === 39) {
            // right arrow, find next sibling
            const ids = await this.fecthNextPage(categoryIdx, focusItem.parentElement.nextElementSibling, focusItem.parentElement.parentElement);
            this.store.updateVideoMap(ids);
            isNextExist = util.goToHorizontalSibling(focusItem.parentElement.nextElementSibling, this.setFocus.bind(this));
            if(isNextExist&&!util.isVisible(focusItem.parentElement.nextElementSibling, focusItem.parentElement.parentElement)){
                util.slidePrevOrNext(util.getDistance(focusItem.parentElement, false), focusItem.parentElement.parentElement, false);
            } 
        }
        if(isNextExist) {
    ...
    renderTmpl(videos, category) {
        ...
        let list = videos.map((v, index) => {
            return `<li><img data-id='${v.videoId}' data-category-id='${category.id}' src='/images/boxart/${v.videoId}.jpg'/></li>`;
        }).join('');
